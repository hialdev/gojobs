    ...
    <div v-if="isReady" class="bg-white">
        <div class="container mx-auto flex items-center justify-between px-5 py-2">
            ...
            <div class="flex items-center gap-3">
                <div class="relative">
                    <!-- Notifications -->
                    <button @click="() => {notifShow = !notifShow; menuShow = false}" type="button" data-dropdown-toggle="notification-dropdown" class="p-2 mr-1 text-gray-500 rounded-lg hover:text-gray-900 hover:bg-orange-50 focus:ring-4 focus:ring-orange-50 relative">
                        <div v-if="notifStore.unreadNotificationsCount != 0" class="absolute top-[0] start-[0] mt-[-2px] ms-[-2px] w-[20px] h-[20px] flex items-center justify-center rounded-[99px] text-[10px] font-semibold bg-primary text-white">
                            {{ notifStore.unreadNotificationsCount }}
                        </div>
                        <!-- Bell icon -->
