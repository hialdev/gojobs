                    ...
                    <div class="bg-white rounded-none md:rounded-2xl p-6">
                        <div v-if="!show.editImageSosmed" class="flex flex-col items-center gap-1 border-b pb-5 relative">
                            ...
                            <div class="flex flex-col items-start justify-start w-full gap-3 my-3">
                                ...
                                <div class="flex items-center gap-x-4">
                                    <NuxtLink :to="`https://twitter.com/${dataStore?.profile?.sosmed?.x}`" target="_blank" class="flex items-center p-2 rounded-xl text-gray-600 bg-gray-100">
                                ...
                                <div class="flex items-center gap-x-4">
                                    <NuxtLink :to="`${dataStore.profile?.sosmed?.others}`" target="_blank" class="flex items-center p-2 rounded-xl text-slate-600 bg-slate-100">
                                    ...
                                    <span class="text-sm">{{dataStore.profile?.sosmed?.others}}</span>
                                </div>
                            </div>
                        </div>
                        <div v-if="show.editImageSosmed" class="flex flex-col items-center md:gap-1 border-b pb-5 relative">
                            <div @click="saveData('editImageSosmed')" class="cursor-pointer absolute z-[20] top-0 end-0 flex items-center justify-center text-slate-300 hover:text-primary">
                        ...
                        <ul v-if="!show.biodata" class="flex flex-col py-5 border-b">
                            ...
                            <li class="flex items-center gap-4 text-sm">
                                ...
                                <span class="">{{cityStore.getCityName(dataStore.biodata?.birth_place) }}, {{dataStore.biodata.birth_date == null ? 'YYYY - MM - DD' : formatDate(dataStore.biodata.birth_date)}}</span>
                            </li>
                            <li class="flex items-center gap-4 text-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
                        ...
                        <ul v-if="!show.informasi" class="flex flex-col py-5">
                            ...
                            <li class="flex items-center gap-4 text-sm">
                                ...
                                <span class="">{{cityStore.getProvinceName(dataStore.profile?.province)}}</span>
                            </li>
                            <li class="flex items-center gap-4 text-sm">
                                <div>
                                ...
                                <span class="">{{cityStore.getCityName(dataStore.profile?.city)}}</span>
                            </li>
                            ...
                            <li class="flex items-center gap-4 text-sm">
                                ...
                                <span class="">{{dataStore.profile?.postal_code}}</span>
                            </li>
                        </ul>
                        <ul v-if="show.informasi" class="flex flex-col py-5">
                            <li class="flex items-center justify-between">
                            ...
                            <div v-if="show.skill" class="grid grid-cols-12 gap-x-5 py-3 mb-5">
                                <div class="col-span-12">
                                    <div class="z-10">
                                        ...
                                        <PartialsMultiselect :selectedOptions="singleData?.skills" @selected="selectSkill" :svgData="skillSvg" :options="skillStore.options" class="z-10 border text-sm border-slate-200 rounded-3xl mb-3" :placeholder="`Skills`" :label="`Skills`" />
...
onMounted(async () => {
    ...
    dataStore.value.languages = sortedData('languages').value;
...
const selectSkill = (value) => {
    ...
    console.log(value);
    singleData.value.skills = value.map(item => item.key);
}
